...
def fast_clear_noise(data,name,base,jumps):
	save_folder = os.path.join(base,name)
	try:
		os.mkdir(save_folder)
	except FileExistsError as ex:
	...
	for i in tqdm(correct):
		plt.plot(i[::jumps], alpha=0.2)
	mean=np.mean(correct,axis=0)
	plt.plot(mean[::jumps],color='black')
	...
	pickle.dump({
		name+'_correct': correct,
		'rest':rest_list
	}, open('data/'+name+'_correct.p', 'wb'))
	pickle.dump({
		name+'_mean': mean,
	}, open('data/'+name+'_mean.p', 'wb'))
	pickle.dump({
		'rest':mean(rest_list),
	}, open(base + '/2017_05_08_A_4-5_stable_conc_aligned_selected_Moria' + '_data.p', 'wb'))
	return correct, mean
