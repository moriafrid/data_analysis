...
if do_calculate_F_factor:
    ...
    spine_neck_L=0.782
initial_folder = "data/fit/const_param"
try:os.mkdir(initial_folder)
except FileExistsError:pass
def change_model_pas(CM=1, RA = 250, RM = 20000.0, E_PAS = -70.0, F_factor = 2.03):
    h.dt = 0.1
...
def fit2short_pulse_CM_const(cell,short_pulse,folder="",CM=1,RM=10000,RA=100):
    global CM_IX,RM_IX, RA_IX
    CM_IX = -1
    RM_IX=0
    RA_IX = 1
    opt_vals = h.Vector(2)
    ...
    for i in range(3):
        ...
        plot_res(CM=CM, RM=RM, RA=RA, save_folder=folder,save_name="_fit_after_" + str(i + 1))
    pickle.dump({
        "RM": RM,
        "RA": RA,
        "CM": CM,
        "error":RMSD
    }, open(folder.split('/')[-1]+"_fit_result.p", "wb"))
    return
...
if __name__=='__main__':
    ...
    cm_folder = initial_folder+"/CM_const"
    ...
    CM_const=cm
    solution_CM_const={}
    folder = cm_folder+"/CM_const="+str(CM_const)
    try:os.mkdir(folder)
    except FileExistsError:pass
    solution_CM_const["CM_const="+str(cm)]=fit2short_pulse_CM_const(cell,short_pulse,folder=folder,CM=CM_const,RM=rm,RA=RA)
    pickle.dump(solution_CM_const , open(cm_folder+"_fit_results.p", "wb"))
    CM_const = 1.0
    ...
    # pickle.dump(solution_RM0 , open(rm_folder+"_fit_results.p", "wb"))
    #
    # ra_folder = initial_folder + "/RA0"
    # try:os.mkdir(ra_folder)
    # except FileExistsError:pass
    # RAs = [50,100,150,200,250,300]
    # solution_RA0={}
    # for ra in [RAs[1]]:
    ...
    #     for rm in RMs:
    #         for ra in RAs:
    #             folder = CM_RA_RM_folder + "/CM0="+str(cm)+"_RM0="+str(rm)+"_RA0=" + str(ra)
    #             try:os.mkdir(folder)
    #             except FileExistsError:pass
    #             solution["CM0="+str(cm)+" RM0="+str(rm)+" RA0=" + str(ra)] = fit2short_pulse(cell, short_pulse, folder=folder, CM=cm, RM=rm, RA=ra)
    #
    # pickle.dump(solution, open(rm_folder + "_fit_results.p", "wb"))
